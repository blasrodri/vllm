...
impl<T: Tokenizer + ?Sized> DecodeStream<'_, T> {
    ...
    ///
    /// This suffix shortcut is only sound for context-independent (byte-level)
    /// decoders, where a token decodes to the same bytes regardless of its
    /// left context. Context-dependent decoders (Metaspace/SentencePiece)
    /// render a token differently by position, so priming from a suffix can
    /// drop tokens; for those we fall back to priming from the full prompt.
    fn seed_prefix(&mut self) -> Result<()> {
        let prompt_len = self.ids.len();
        if self.tokenizer.decode_is_context_independent() && prompt_len > SAFE_SUFFIX_MIN {
...
mod tests {
    ...
    impl Tokenizer for Utf8Backend {
        ...
        // Raw bytes decode identically regardless of position.
    ...
    /// Byte-level backend priming from a trailing suffix (prompt longer than
    /// `SAFE_SUFFIX_MAX`) must stream the generated text correctly. Exercises
    /// the suffix-seed fast path enabled by `decode_is_context_independent`.
    #[test]
    fn byte_level_suffix_seed_streams_correctly() {
        let backend = Utf8Backend;
        let prompt: Vec<u32> = b"abcdefgh".iter().map(|&b| b as u32).collect();
        let mut decoder = backend.create_decode_stream(&prompt, false, 0);
        ...
        let (last_chunk, _full) = decoder.flush(None).unwrap();
        if let Some(chunk) = last_chunk {
            out.push_str(&chunk);
        }
        // Prompt context is not re-emitted; only generated bytes stream out.
        assert_eq!(out, "XYZ");
    }

    /// Backend simulating a context-dependent decoder (Metaspace/SentencePiece):
    /// a token's rendered length depends on its left context, so decoding a
    /// truncated suffix does not match the corresponding tail of the full
    /// decode. Priming the prefix from a suffix would drop the final token.
    ///
    /// `context_independent` lets the same decode masquerade as byte-level to
    /// show the suffix-seed shortcut is what drops the token.
    #[derive(Debug)]
    struct ContextDependentBackend {
        context_independent: bool,
    }

    impl Tokenizer for ContextDependentBackend {
        fn encode(&self, _text: &str, _add_special_tokens: bool) -> Result<Vec<u32>> {
            unreachable!()
        }
        ...
        fn decode(&self, token_ids: &[u32], _skip_special_tokens: bool) -> Result<String> {
            match token_ids {
                // Full prompt and full sequence: token 6 renders as "!".
                [1, 2, 3, 4, 5] => Ok("ABCDE".into()),
                [1, 2, 3, 4, 5, 6] => Ok("ABCDE!".into()),
                // Trailing suffix (token 1 dropped): without the leading
                // context, token 6 contributes nothing, so appending it does
                // not grow the string — the suffix-seed path would drop it.
                [2, 3, 4, 5] => Ok("BCDE".into()),
                [2, 3, 4, 5, 6] => Ok("BCDE".into()),
                [6] => Ok("!".into()),
    ...
    /// With the decoder correctly reported as context-dependent, the generated
    /// token is streamed instead of silently dropped.
    #[test]
    fn context_dependent_decoder_does_not_drop_token() {
        let backend = ContextDependentBackend {
            context_independent: false,
        };
        let mut decoder = backend.create_decode_stream(&[1, 2, 3, 4, 5], false, 0);

        decoder.push_token(6).unwrap();
        let (_last, full) = decoder.flush(None).unwrap();
        assert_eq!(full, "!");
    }

    /// Same decode mislabeled as context-independent takes the suffix-seed
    /// shortcut and drops the final token — the failure the gating prevents.
    #[test]
    fn suffix_seed_drops_token_for_context_dependent_decode() {
        let backend = ContextDependentBackend {
